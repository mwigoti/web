...
const alertVariants = cva(
  "relative w-full rounded-xl border p-4 text-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground [&>svg~*]:pl-7",
  {
    variants: {
      variant: {
        default: "bg-[#182C29] border-[#2B4543] text-[#D6E3DE]",
        destructive:
          "border-red-900/50 text-red-300 bg-red-950/30 [&>svg]:text-red-400",
        lime:
          "border-[#CFF4A7]/30 bg-[#CFF4A7]/10 text-[#CFF4A7] [&>svg]:text-[#CFF4A7]",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("text-xs text-[#D6E3DE] leading-relaxed [&_p]:leading-relaxed", className)}
    {...props}
  />
));
